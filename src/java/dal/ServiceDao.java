...
public class ServiceDao extends DBContext {
    ...
    public Service getServiceById(int serviceId) {
        String sql = """
        SELECT ServiceID, ServiceName, Description, Price, Duration, 
               IsActive, CreatedBy, CreatedDate, Image
        FROM dbo.Services 
        WHERE ServiceID = ?
    """;
    ...
    public List<Service> getAllServices() {
        ...
        SELECT s.ServiceID, s.ServiceName, s.Description, s.Price,
               s.Duration, s.IsActive, s.CreatedBy, s.CreatedDate, s.Image,
               u.FullName
        FROM dbo.Services s
    ...
    SELECT s.ServiceID, s.ServiceName, s.[Description], s.Price, s.Duration,
           s.IsActive, s.CreatedBy, s.CreatedDate, s.Image,
...
""");
        ...
        //mo ket noi
        try (Connection connect = new DBContext().connection; PreparedStatement ps = connect.prepareStatement(sql.toString())) {
    ...
    public List<Service> filterServices(ServiceDto filter) {
    ...
    """);
        ...
        // Sort (ASC/DESC)
        sql.append(" ORDER BY s.ServiceID ").append(filter.isSortMode() ? "ASC" : "DESC");
    ...
    public int countFilter(ServiceDto filter) {
        int count = 0;
    ...
    """);
        ...
        // Điều kiện lọc giống filterService
        if (filter.getServiceName() != null && !filter.getServiceName().isBlank()) {
            sql.append(" AND s.ServiceName LIKE ?\n");
        }
        if (filter.getPriceFrom() != null) {
